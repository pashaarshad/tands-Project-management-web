...
@section('title', 'Developer Attendance')
...
@section('content')
    <main class="page-area">
        ...
        <div class="page-header">
            <div>
                <h1 class="page-title">Developer Team Attendance</h1>
                <p class="page-desc">Comprehensive log of all work presence for the development department.</p>
                <div class="mt-2">
        ...
        <div class="kpi-grid mb-4" style="display:grid; grid-template-columns: repeat(5, 1fr); gap:20px;">
             <div class="kpi-card" style="--kpi-accent:#6366f1">
                <div class="kpi-top">
                    <div class="kpi-icon" style="background:rgba(99,102,241,.15);color:#6366f1"><i class="bi bi-person-workspace"></i></div>
                </div>
            ...
            <div class="kpi-card" style="--kpi-accent:#10b981">
                <div class="kpi-top">
                    <div class="kpi-icon" style="background:rgba(16,185,129,.15);color:#10b981"><i class="bi bi-patch-check-fill"></i></div>
                </div>
            ...
            <div class="kpi-card" style="--kpi-accent:#f59e0b">
                <div class="kpi-top">
                    <div class="kpi-icon" style="background:rgba(245,158,11,.15);color:#f59e0b"><i class="bi bi-exclamation-square-fill"></i></div>
                </div>
        ...
        <div class="dash-card">
            <div class="card-head" style="align-items: center; gap: 12px; flex-wrap: wrap;">
                <div class="card-title">All Developers</div>
                    ...
                    <form action="{{ route('admin.attendance.dev-index') }}" method="GET" class="card-actions mb-0" id="filterForm" style="display:flex; align-items:center; gap:8px;">
                        ...
                        <select name="user_id" class="filter-select" onchange="document.getElementById('filterForm').submit()" style="height:38px; min-width:160px;">
                            ...
                            @foreach($allDevelopers as $dev)
                                <option value="{{ $dev->id }}" {{ request('user_id') == $dev->id ? 'selected' : '' }}>{{ $dev->name }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="modal-body px-4 py-4">
                        <div class="row g-3">
                            <div class="col-md-6">
                                ...
                                <input type="time" name="dev_checkin_time" class="form-control" value="{{ $settings->dev_checkin_time }}">
                            </div>
                            <div class="col-md-6">
                                <label class="form-label text-muted small fw-bold">Dev Check-out</label>
                                <input type="time" name="dev_checkout_time" class="form-control" value="{{ $settings->dev_checkout_time }}">
                            </div>
                            <!-- Preserve sale times too -->
                            <input type="hidden" name="sale_checkin_time" value="{{ $settings->sale_checkin_time }}">
                            <input type="hidden" name="sale_checkout_time" value="{{ $settings->sale_checkout_time }}">
                            <div class="col-12">
