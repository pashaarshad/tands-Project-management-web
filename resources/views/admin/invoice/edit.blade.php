...
@section('content')
    ...
    <div class="page" id="page-invoice-edit">
        <div class="page-header">
            <div>
                <h1 class="page-title">Edit Invoice</h1>
                <p class="page-desc">Modify details for #{{ $invoice->invoice_no }}</p>
            </div>
        ...
        <form action="{{ route($routePrefix . '.invoices.update', $invoice->id) }}" method="POST" id="invoiceForm">
            @csrf
            @method('PUT')
            <div class="form-grid-2-1">
                            ...
                            <div class="form-grid">
                                ...
                                <div class="form-row">
                                    ...
                                    <div style="display: flex; align-items: stretch;">
                                        ...
                                        <input type="text" name="invoice_no" value="{{ old('invoice_no', $invoice->invoice_no) }}" class="form-inp" style="border-top-left-radius: 0; border-bottom-left-radius: 0; flex: 1; height: 42px;" oninput="this.value = this.value.replace(/[^0-9]/g, '')" maxlength="10" inputmode="numeric" required>
                                    </div>
                                </div>
                                <div class="form-row">
                                    <label class="form-lbl">Invoice Date <span class="text-danger">*</span></label>
                                    <input type="date" name="invoice_date" value="{{ old('invoice_date', $invoice->invoice_date->format('Y-m-d')) }}" class="form-inp" required>
                                </div>
                                <div class="form-row">
                                    <label class="form-lbl">Due Date</label>
                                    <input type="date" name="due_date" value="{{ old('due_date', $invoice->due_date ? $invoice->due_date->format('Y-m-d') : '') }}" class="form-inp">
                                </div>
                                <div class="form-row" style="grid-column: 1/-1;">
                                    <label class="form-lbl">Client Name <span class="text-danger">*</span></label>
                                    <input type="text" name="client_name" id="client_name" value="{{ old('client_name', $invoice->client_name) }}" class="form-inp" required>
                                </div>
                                <div class="form-row" style="grid-column: 1/-1;">
                                    <label class="form-lbl">Client Address</label>
                                    <textarea name="client_address" id="client_address" class="form-inp" rows="2">{{ old('client_address', $invoice->client_address) }}</textarea>
                                </div>
                                <div class="form-row">
                                    <label class="form-lbl">Client GSTIN</label>
                                    <input type="text" name="client_gstin" value="{{ old('client_gstin', $invoice->client_gstin) }}" class="form-inp">
                                </div>
                                <div class="form-row">
                        ...
                        <div class="card-body" style="padding: 0;">
                            <table class="table" id="itemsTable">
                                ...
                                <tbody>
                                    ...
                                    <tr class="item-row">
                                        <td><input type="text" name="items[{{ $idx }}][desc]" class="form-inp sm" value="{{ $item['desc'] }}" required></td>
                                        <td><input type="text" name="items[{{ $idx }}][hsn]" class="form-inp sm" value="{{ $item['hsn'] ?? '' }}"></td>
                                        <td><input type="number" name="items[{{ $idx }}][qty]" class="form-inp sm qty" value="{{ $item['qty'] }}" min="1" step="any" oninput="calcRow(this)"></td>
                                        <td><input type="number" name="items[{{ $idx }}][rate]" class="form-inp sm rate" value="{{ $item['rate'] }}" min="0" step="any" oninput="calcRow(this)"></td>
                                        <td><input type="number" name="items[{{ $idx }}][amount]" class="form-inp sm amount" value="{{ $item['amount'] }}" readonly></td>
                                        <td>
                                            @if($idx > 0)
                                                <button type="button" class="text-danger" onclick="this.closest('tr').remove(); calcTotals();" style="border:none; background:none; cursor:pointer;"><i class="bi bi-x-circle"></i></button>
                                            @endif
                                        </td>
                                    </tr>
                                    @endforeach
                ...
                <div class="right-col" style="display: flex; flex-direction: column; gap: 20px;">
                        ...
                        <div class="card-body">
                            <div style="display: flex; flex-direction: column; gap: 15px;">
                                <div class="summary-item" style="display: flex; justify-content: space-between; font-weight: 500;">
                                    <span>Subtotal</span>
                                    <span>₹<span id="subtotal_text">{{ number_format($invoice->subtotal, 2) }}</span></span>
                                    <input type="hidden" name="subtotal" id="subtotal_val" value="{{ $invoice->subtotal }}">
                                </div>
                                <div class="summary-item">
                                    <label class="form-lbl">CGST (%)</label>
                                    <div style="display: flex; gap: 10px; align-items: center;">
                                        @php $cgst_p = $invoice->subtotal > 0 ? ($invoice->cgst / $invoice->subtotal) * 100 : 0; @endphp
                                        <input type="number" id="cgst_p" class="form-inp sm" value="{{ round($cgst_p, 2) }}" oninput="calcTotals()">
                                        <span id="cgst_text" style="min-width: 60px; text-align: right;">{{ number_format($invoice->cgst, 2) }}</span>
                                        <input type="hidden" name="cgst" id="cgst_val" value="{{ $invoice->cgst }}">
                                    </div>
                                    <!-- Sender Details -->
                    <div class="dash-card">
                        <div class="card-head">
                        ...
                        <div class="card-body">
                            <div class="form-grid">
                                ...
                                <div class="form-row">
                                    ...
                                    <input type="text" name="sender_gstin" value="{{ old('sender_gstin', $invoice->sender_gstin) }}" class="form-inp" placeholder="29JTKPS5068C1Z1">
                                </div>
                                <div class="form-row" style="grid-column: 1/-1;">
                                    <label class="form-lbl">Address</label>
                                    <textarea name="sender_address" class="form-inp" rows="2" placeholder="KannamangalaPost, Whitefield Main Road, Bengaluru Rural, Karnataka 560067">{{ old('sender_address', $invoice->sender_address) }}</textarea>
                                </div>
                                <div class="form-row">
                                    <label class="form-lbl">Contact No</label>
                                    <input type="text" name="sender_contact" value="{{ old('sender_contact', $invoice->sender_contact) }}" class="form-inp" placeholder="+91 86606 32597">
                                </div>
                                <div class="form-row">
                                    <label class="form-lbl">Email ID</label>
                                    <input type="email" name="sender_email" value="{{ old('sender_email', $invoice->sender_email) }}" class="form-inp" placeholder="zentrics@gmail.com">
                                </div>
                            </div>
                        </div>
                    </div>
                                ...
                                <div class="summary-item">
                                    ...
                                    <div style="display: flex; gap: 10px; align-items: center;">
                                        @php $sgst_p = $invoice->subtotal > 0 ? ($invoice->sgst / $invoice->subtotal) * 100 : 0; @endphp
                                        <input type="number" id="sgst_p" class="form-inp sm" value="{{ round($sgst_p, 2) }}" oninput="calcTotals()">
                                        <span id="sgst_text" style="min-width: 60px; text-align: right;">{{ number_format($invoice->sgst, 2) }}</span>
                                        <input type="hidden" name="sgst" id="sgst_val" value="{{ $invoice->sgst }}">
                                    </div>
                                </div>
                                <div class="summary-item">
                                    <label class="form-lbl">IGST (%)</label>
                                    <div style="display: flex; gap: 10px; align-items: center;">
                                        @php $igst_p = $invoice->subtotal > 0 ? ($invoice->igst / $invoice->subtotal) * 100 : 0; @endphp
                                        <input type="number" id="igst_p" class="form-inp sm" value="{{ round($igst_p, 2) }}" oninput="calcTotals()">
                                        <span id="igst_text" style="min-width: 60px; text-align: right;">{{ number_format($invoice->igst, 2) }}</span>
                                        <input type="hidden" name="igst" id="igst_val" value="{{ $invoice->igst }}">
                                    </div>
                                </div>
                                <div class="summary-item">
                                    <label class="form-lbl">Adjustment</label>
                                    <input type="number" name="adjustment" id="adjustment" class="form-inp sm" value="{{ $invoice->adjustment }}" step="any" oninput="calcTotals()">
                                </div>
                                ...
                                <div class="summary-item" style="display: flex; justify-content: space-between; font-weight: 800; font-size: 18px; color: var(--navy);">
                                    <span>Total</span>
                                    <span>₹<span id="total_text">{{ number_format($invoice->total, 2) }}</span></span>
                                    <input type="hidden" name="total" id="total_val" value="{{ $invoice->total }}">
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="dash-card">
                        ...
                        <div class="card-body">
                            <textarea name="notes" class="form-inp" rows="3" placeholder="Additional notes...">{{ $invoice->notes }}</textarea>
                        </div>
                    </div>
