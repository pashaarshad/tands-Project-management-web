{{--
    admin/leads/_multiselect_assets.blade.php
...
<style>
    ...
    .ms-trigger:hover,
    .ms-wrap.open .ms-trigger { border-color: var(--accent); background: var(--bg2); }
    .ms-wrap.open .ms-trigger { box-shadow: 0 0 0 3px rgba(99,102,241,.1); }

    .ms-pills { display: flex; flex-wrap: wrap; gap: 4px; flex: 1; min-width: 0; }
    .ms-placeholder { font-size: 13px; color: var(--t4); }
    ...
    .ms-pill-x { font-size: 10px; color: var(--accent); cursor: pointer; opacity:.7; line-height:1; padding:0 0 0 2px; }
    .ms-pill-x:hover { opacity: 1; }

    .ms-arrow { font-size: 11px; color: var(--t3); transition: transform .2s ease; flex-shrink: 0; }
    ...
    .ms-search-wrap { display:flex; align-items:center; gap:8px; padding:8px 12px; border-bottom:1px solid var(--b1); }
    .ms-search-wrap i { color:var(--t3); font-size:12px; flex-shrink:0; }
    .ms-search { background:none; border:none; outline:none; font-size:13px; color:var(--t1); font-family:var(--font); width:100%; }
    .ms-search::placeholder { color:var(--t4); }

    .ms-opts { max-height:320px; overflow-y:auto; padding:4px; scrollbar-width:thin; scrollbar-color:var(--b2) transparent; }
    .ms-opt {
    ...
    .ms-opt:hover { background:var(--bg4); color:var(--t1); }
