...
@section('title', 'Update Sales Person')
    ...
    <div class="page" id="page-dashboard">
        ...
        <div class="dash-grid">
            <div class="dash-card span-12">
                <div class="card-head">
                    <div class="card-title">Sales Person Details</div>
                </div>
                <div class="card-body">
                    <form action="{{ route($routePrefix . '.sales-person.update', $sale->id) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        <div class="form-grid">
                            <div class="form-row">
                                <label class="form-lbl">Sales Person *</label>
                                <input type="text" name="name" class="form-inp" value="{{ old('name', $sale->name) }}" required>
                            </div>
                            <div class="form-row">
                                <label class="form-lbl">Email *</label>
                                <input type="email" name="email" class="form-inp" value="{{ old('email', $sale->email) }}" required>
                            </div>
                            <div class="form-row">
                            ...
                            <div class="form-row">
                                ...
                                <input type="text" name="phone" class="form-inp" value="{{ old('phone', $sale->phone) }}" placeholder="Phone Number">
                            </div>
                            <div class="form-row">
                                <label class="form-lbl">Address</label>
                                <textarea name="address" class="form-inp" rows="2" placeholder="Address">{{ old('address', $sale->address) }}</textarea>
                            </div>
                            ...
                            <div class="form-row">
                                ...
                                @if($sale->aadhar_card)
                                    <div style="margin-bottom: 8px;">
                                        <a href="{{ asset('storage/' . $sale->aadhar_card) }}" target="_blank" style="color: var(--accent); font-size: 13px;"><i class="bi bi-file-earmark-text"></i> View Current File</a>
                                    </div>
                                @endif
                            ...
                            <div class="form-row">
                                ...
                                @if($sale->pan_card)
                                    <div style="margin-bottom: 8px;">
                                        <a href="{{ asset('storage/' . $sale->pan_card) }}" target="_blank" style="color: var(--accent); font-size: 13px;"><i class="bi bi-file-earmark-text"></i> View Current File</a>
                                    </div>
                                @endif
                            ...
                            <div class="form-row">
                                ...
                                @if($sale->voter_card)
                                    <div style="margin-bottom: 8px;">
                                        <a href="{{ asset('storage/' . $sale->voter_card) }}" target="_blank" style="color: var(--accent); font-size: 13px;"><i class="bi bi-file-earmark-text"></i> View Current File</a>
                                    </div>
                                @endif
                            ...
                            <div class="form-row">
                                ...
                                @if($sale->bank_account_pic)
                                    <div style="margin-bottom: 8px;">
                                        <a href="{{ asset('storage/' . $sale->bank_account_pic) }}" target="_blank" style="color: var(--accent); font-size: 13px;"><i class="bi bi-file-earmark-text"></i> View Current File</a>
                                    </div>
                                @endif
                            ...
                            <div class="form-row">
                                ...
                                <textarea name="qualification_details" class="form-inp" rows="2">{{ old('qualification_details', $sale->qualification_details) }}</textarea>
                            </div>

                            <div class="form-row">
                                <label class="form-lbl">Qualification Attachments (Multiple)</label>
                                @if($sale->qualification_attachments && count($sale->qualification_attachments) > 0)
                                    <div style="margin-bottom: 8px; display: flex; gap: 8px; flex-wrap: wrap;">
                                        @foreach($sale->qualification_attachments as $index => $attachment)
                        ...
                        <div style="margin-top: 30px; display: flex; gap: 12px; padding-top: 20px; border-top: 1px solid var(--b2);">
                            ...
                            <a href="{{ route($routePrefix . '.sales-person') }}" class="btn-ghost">Cancel</a>
                        </div>
                    </form>
                </div>
            </div>
